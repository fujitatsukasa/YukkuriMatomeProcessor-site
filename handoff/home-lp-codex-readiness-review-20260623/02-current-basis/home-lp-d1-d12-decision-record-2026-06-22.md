...
D1〜D11の実回答は `docs/home-lp-d1-d11-answer-form-2026-06-23.md` を使って回収します。回答フォームのコード上の正本は `vite-site/src/data/product-facts.ts` の `decisionAnswerRequirements` です。
...
`vite-site/src/data/product-facts.ts` の `releaseCandidateDistribution` に、0.0.18系の配布候補情報を集約しています。

これはD10確認用の候補値であり、公開配布版の承認済み正本ではありません。

- 配布候補バージョン: `0.0.18候補`
- 状態: `pending`
- 直接取得CTA: `downloadReady` がtrueになるまで表示しない
- ライブ配布メタデータ取得: `downloadReady` がtrueになるまで表示へ反映しない
- SHA256一覧などの外部リンク: D10確認完了後に表示する
