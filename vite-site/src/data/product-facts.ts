...
export type OperationalDecisionId = Exclude<DecisionId, 'D12'>
...
export type DecisionAnswerRequirement = {
...
export const requiredDecisionMetadata = [
  '決定者',
  '根拠資料',
  '適用アプリ版',
  '適用サイトcommit',
...
export const decisionAnswerRequirements = [
  {
    id: 'D1',
    question: '誰に向けたLPで、主要CVは何で、商品が出す成果物と非対応範囲は何か。',
    requiredFields: [...requiredDecisionMetadata, '対象ユーザー', 'LPの役割', '主要CV', '副次CV', '成果物', '非対応範囲'],
    evidenceTypes: ['事業責任者の承認メモ', '実アプリの出力例', 'LP上の表示案'],
    approvalRoles: ['事業責任者'],
    publicCopyRule: '成果物と非対応範囲を同じ近さで表示し、動画完成・投稿・収益を保証しない。',
    blocks: ['publishReady'],
  },
  {
    id: 'D2',
  ...
  {
    id: 'D3',
    question: 'Premiumで追加される権利、残る上限、公正利用、警告・停止条件は何か。',
    requiredFields: [...requiredDecisionMetadata, 'Premium権利', '残る上限', '公正利用条件', '警告条件', '停止条件', 'ユーザー通知方法'],
    evidenceTypes: ['料金画面の表示', '利用規約該当箇所', '権限判定の実装根拠'],
    approvalRoles: ['事業責任者', '法務・データ責任者'],
    publicCopyRule: 'Premiumを「無制限」「制限解除」と表現せず、追加権利と残る制約を並べる。',
    blocks: ['purchaseReady', 'publishReady'],
  },
  {
    id: 'D4',
    question: 'Googleアカウント、ライセンス、PC台数、同時利用、PC変更、再認証の条件は何か。',
  ...
  {
    id: 'D5',
    question: '価格、決済、最終確認、権限付与、失敗時対応、返金、領収書の正式条件は何か。',
    requiredFields: [...requiredDecisionMetadata, '価格', '決済方法', '最終確認画面', '権限付与時間', '失敗時対応', '返金条件', '領収書/請求書'],
    evidenceTypes: ['Stripe Checkout設定', '権限付与E2E結果', '返金ポリシー', '特商法表示'],
    approvalRoles: ['事業責任者', '法務・データ責任者'],
    publicCopyRule: '購入CTAは決済から権限付与までのE2Eと返金条件が揃うまで表示しない。',
    blocks: ['purchaseReady', 'publishReady'],
  },
  {
    id: 'D6',
    question: '買い切りに含まれる更新、保守、サポート、メジャー版、EOL条件は何か。',
    requiredFields: [...requiredDecisionMetadata, '更新範囲', '保守範囲', 'サポート対象', 'メジャー版扱い', 'EOL条件', '告知方法'],
    evidenceTypes: ['利用規約該当箇所', 'サポートポリシー', '更新履歴の運用案'],
    approvalRoles: ['事業責任者', '法務・データ責任者'],
    publicCopyRule: '買い切りを表示する場合、更新・保守の範囲を過大に見せない。',
    blocks: ['purchaseReady', 'publishReady'],
  },
  {
    id: 'D7',
    question: '対応入力、対応URL、取得範囲、非対応条件、権利・出典確認の扱いは何か。',
    requiredFields: [...requiredDecisionMetadata, '対応入力', '対応URL', '取得範囲', '非対応条件', '取得失敗時挙動', '権利/出典確認責任'],
    evidenceTypes: ['対応URL一覧', '取得成功/失敗サンプル', '権利確認の注意文'],
    approvalRoles: ['アプリ・リリース責任者', '法務・データ責任者'],
    publicCopyRule: '対応URLを広く見せる場合は、非公開/ログイン必須/ペイウォール等の非対応条件も表示する。',
    blocks: ['publishReady'],
  },
  {
    id: 'D8',
    question: 'YMM4対応版、出力形式、受渡手順、動作環境、互換性検証の結果は何か。',
    requiredFields: [...requiredDecisionMetadata, 'YMM4対応版', '出力形式', '受渡手順', 'Windows要件', 'WebView2等の要件', '互換性検証結果'],
    evidenceTypes: ['実YMM4での受渡確認', '出力ファイル例', '動作環境チェック結果'],
    approvalRoles: ['アプリ・リリース責任者'],
    publicCopyRule: 'YMM4連携を表示する場合、対応版と仕上げがYMM4側で必要なことを併記する。',
    blocks: ['downloadReady', 'publishReady'],
  },
  {
    id: 'D9',
    question: '外部AI/API、送信データ、保持・削除、分析・テレメトリーの方針は何か。',
    requiredFields: [...requiredDecisionMetadata, '外部送信先', '送信データ', '保持期間', '削除方法', '分析/テレメトリー', 'ユーザー表示方法'],
    evidenceTypes: ['プライバシーポリシー', 'API送信箇所の実装根拠', 'ユーザー向け説明文'],
    approvalRoles: ['法務・データ責任者', 'アプリ・リリース責任者'],
    publicCopyRule: '外部AI/APIを便利機能として出す場合、送信データとユーザー確認責任を隠さない。',
    blocks: ['publishReady'],
  },
  {
    id: 'D10',
  ...
  {
    id: 'D11',
    question: '主張・証拠対応、実画面、実出力、動画、サンプル、素材権利の承認状態は何か。',
    requiredFields: [...requiredDecisionMetadata, '販売主張', '対応証拠', '撮影元アプリ版', '素材出典', '掲載許可', 'マスキング', '有効期限'],
    evidenceTypes: ['主張・証拠対応表', '実画面スクリーンショット', '実出力ファイル', '動画/サンプル権利メモ'],
    approvalRoles: ['事業責任者', '法務・データ責任者', 'アプリ・リリース責任者'],
    publicCopyRule: '架空UIやAI生成UIを商品証拠として使わず、主張ごとに実証拠を対応させる。',
    blocks: ['publishReady'],
  },
] as const satisfies readonly DecisionAnswerRequirement[]
